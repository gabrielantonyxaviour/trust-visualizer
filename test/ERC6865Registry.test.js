...
it("Should add and get implementation", async function () {
  ...
  it("Should fail to add implementation from non-owner", async function () {
    ...
    await expect(registry.connect(notOwnerAddress).addImplementation(domainSeparator, implementationAddress)).to.be.revertedWith("Ownable: caller is not the owner");
  });
  ...
  it("Should fail to add implementation with zero address", async function () {
    ...
    await expect(registry.connect(owner).addImplementation(domainSeparator, zeroAddress)).to.be.revertedWith("Invalid implementation address");
  });
});